...
public class MqttAndroidClient extends BroadcastReceiver implements
     ...
     * @param clientId    specifies the name by which this connection should be
     *                    identified to the server
     * @param persistence The object to use to store persisted payload
     */
    ...
    public boolean isConnected() {
        ...
        return clientHandle != null && mMqttService != null && mMqttService.isConnected(clientHandle);
    }
    ...
    public void close() {
        if (mMqttService != null) {
            if (clientHandle == null) {
                clientHandle = mMqttService.getClient(serverURI, clientId, myContext.getApplicationInfo().packageName, persistence);
            }
            mMqttService.close(clientHandle);
        }
    }
    ...
    public IMqttToken connect(MqttConnectOptions options, Object userContext,
         ...
         */
        if (mMqttService == null) { // First time - must bind to the service
    ...
    private void doConnect() {
        if (null != mMqttService) {
            if (clientHandle == null) {
                clientHandle = mMqttService.getClient(serverURI, clientId, myContext.getApplicationInfo().packageName,
                        persistence);
            }
            mMqttService.setTraceEnabled(traceEnabled);
            mMqttService.setTraceCallbackId(clientHandle);

            String activityToken = storeToken(connectToken);
            try {
                mMqttService.connect(clientHandle, connectOptions, null,
                        activityToken);
    ...
    public IMqttToken disconnect() throws MqttException {
        ...
        if (null != mMqttService)
            mMqttService.disconnect(clientHandle, null, activityToken);
        return token;
    }
    ...
    public IMqttToken disconnect(long quiesceTimeout) throws MqttException {
        ...
        String activityToken = storeToken(token);
        if (null != mMqttService)
            mMqttService.disconnect(clientHandle, quiesceTimeout, null,
                    activityToken);
        return token;
    }
    ...
    public IMqttDeliveryToken publish(String topic, byte[] payload, int qos,
        ...
        String activityToken = storeToken(token);
        if (null != mMqttService) {
            IMqttDeliveryToken internalToken = mMqttService.publish(clientHandle,
    ...
    public IMqttToken subscribe(String topic, int qos, Object userContext,
        ...
        String activityToken = storeToken(token);
        if (null != mMqttService)
            mMqttService.subscribe(clientHandle, topic, qos, null, activityToken);
        return token;
    }
    ...
    public IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext, IMqttActionListener callback, IMqttMessageListener[] messageListeners) throws MqttException {
        ...
        String activityToken = storeToken(token);
        if (null != mMqttService)
            mMqttService.subscribe(clientHandle, topicFilters, qos, null, activityToken, messageListeners);
        return null;
    }
    ...
    public IMqttToken unsubscribe(String topic, Object userContext,
        ...
        String activityToken = storeToken(token);
        if (null != mMqttService)
            mMqttService.unsubscribe(clientHandle, topic, null, activityToken);
        return token;
    }
    ...
    public IMqttDeliveryToken[] getPendingDeliveryTokens() {
        return mMqttService.getPendingDeliveryTokens(clientHandle);
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (MqttServiceConstants.CONNECT_ACTION.equals(action)) {
            ...
            mMqttService.traceError(MqttService.TAG, "Callback action doesn't exist.");
        }
    ...
    public boolean acknowledgeMessage(String messageId) {
        if (messageAck == Ack.MANUAL_ACK) {
            Status status = mMqttService.acknowledgeMessageArrival(clientHandle, messageId);
    ...
    private void simpleAction(IMqttToken token, Bundle data) {
        if (token != null) {
            ...
            mMqttService.traceError(MqttService.TAG, "simpleAction : token is null");
        }
    }
        ...
        if (callback != null) {
            ...
            try {
                if (messageAck == Ack.AUTO_ACK) {
                    ...
                    mMqttService.acknowledgeMessageArrival(clientHandle, messageId);
                } else {
    ...
    /**
     * Process trace action - pass trace payload back to the callback
     *
     * @param data
     */
    ...
    public void setBufferOpts(DisconnectedBufferOptions bufferOpts) {
        mMqttService.setBufferOpts(clientHandle, bufferOpts);
    }

    public int getBufferedMessageCount() {
        return mMqttService.getBufferedMessageCount(clientHandle);
    }

    public MqttMessage getBufferedMessage(int bufferIndex) {
        return mMqttService.getBufferedMessage(clientHandle, bufferIndex);
    }

    public void deleteBufferedMessage(int bufferIndex) {
        mMqttService.deleteBufferedMessage(clientHandle, bufferIndex);
    }
