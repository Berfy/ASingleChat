...
public class PSTakePhotoUtil {
    ...
    public void choosePhotoFromCamera() {
        ...
        try {
            ...
            e.printStackTrace();
            ToastUtil.getInstances().showShort("获取相机失败");
        }
    }
    ...
    private Intent getCameraIntent(File file) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri imageUri = FileProvider.getUriForFile(mActivity, mActivity.getPackageName() + ".fileproviders", file);//通过FileProvider创建一个content类型的Uri
