...
package cn.berfy.sdk.mvpbase.util.toast;
...
import cn.berfy.sdk.mvpbase.util.toast.utils.AnimationUtils;
...
class Toaster extends Handler {
    ...
    private void displaySuperToast(SuperToast superToast) {
        ...
        if (superToast instanceof SuperActivityToast) {
            ...
            if (windowManager != null) {//android8.0以上系统 弹出框必须加上TYPE_APPLICATION_OVERLAY  8.0以下是TYPE_TOAST
                WindowManager.LayoutParams layoutParams = superToast.getWindowManagerParams();
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    if (Settings.canDrawOverlays(superToast.getContext())) {
                        LogF.d("吐司", "8.0以上系统");
                        LogF.d("吐司", "权限canDrawOverlays = true");
                        layoutParams.type = WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY;
                    } else {
                        LogF.d("吐司", "权限canDrawOverlays = false");
                        //执行系统默认toast
                        showDefaultToast(superToast);
                        return;
                    }
                } else {
                    LogF.d("吐司", "8.0以下系统");
                ...
                try {
                    windowManager.addView(superToast.getView(), layoutParams);
                } catch (Exception e) {
                    LogF.d("吐司", "显示发生错误 执行默认toast");
                    //执行系统默认toast
                    showDefaultToast(superToast);
                    return;
                }
    ...
    private void showDefaultToast(SuperToast superToast) {
        LogF.d("吐司", "默认吐司");
        if (null == mToastDefualt) {
            mToastDefualt = Toast.makeText(superToast.getContext(), superToast.getText(),
                    superToast.getDuration() == Style.DURATION_SHORT ? Toast.LENGTH_SHORT : Toast.LENGTH_LONG);
        }
        if (mToastDefualt != null) mToastDefualt.cancel();
        mToastDefualt = Toast.makeText(superToast.getContext(), superToast.getText(),
                superToast.getDuration() == Style.DURATION_SHORT ? Toast.LENGTH_SHORT : Toast.LENGTH_LONG);
        mToastDefualt.show();
    }
